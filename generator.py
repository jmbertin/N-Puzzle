...
def make_puzzle(s, solvable, iterations):
    """
    Generate a puzzle based on the given size, solvability and number of iterations.
    Args:s (int): The size of the puzzle.
        solvable (bool): True for a solvable puzzle, False otherwise.
        iterations (int): The number of moves to make when shuffling the puzzle.
    Returns: list: The generated puzzle.
    """
    def swap_empty(p):
        idx = p.index(0)
        poss = []
...
def make_goal(s):
    """
    Generate a goal state for the puzzle based on the given size.
    Args: s (int): The size of the puzzle.
    Returns: list: The goal state of the puzzle.
    """
    ts = s*s
